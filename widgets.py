...
class QRCheckWidget(QWidget):
    def __init__(self, parent=None, scale_func=None):
        ...
        self.lbl_title.setStyleSheet(f"font-family: 'Pretendard'; font-size: {self.s(30)}px; font-weight: 500; color: black; background: transparent;")
        ...
        self.lbl_desc.setStyleSheet(f"font-family: 'Pretendard'; font-size: {self.s(22)}px; font-weight: 500; color: #858585; background: transparent;")
        self.lbl_desc.adjustSize()
        
        # 전체 영역 클릭 가능하도록 투명 버튼 배치
    ...
    def paintEvent(self, event):
        ...
        # 명세: 체크박스 사이즈 60*60, R값 10
        box_size = self.s(60)
        # 텍스트와 라인을 맞추기 위해 y축에 약간의 오프셋(예: 5px)을 줄 수 있음
        rect = QRect(0, 0, box_size, box_size)
        
        # 체크 여부에 따른 색상 (활성: 보라 계열, 비활성: 회색)
        bg_color = QColor(73, 59, 219)
        if not self.is_checked:
            bg_color = QColor(200, 200, 200)

        painter.setBrush(bg_color)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRoundedRect(rect, self.s(10), self.s(10)) # R값 10 적용
    ...
    def paintEvent(self, event):
        ...
        painter.setBrush(bg_color)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRoundedRect(rect, self.s(10), self.s(10))  # 🔥 R10
...
class GlobalTimerWidget(QWidget):
    def __init__(self, parent=None, scale_func=None):
        ...
        self.lbl_title.setStyleSheet(f"color: #828282; font-family: 'Pretendard'; font-size: {self.s(24)}px; font-weight: bold; border: none; background: transparent;")
        ...
        self.lbl_num.setStyleSheet(f"color: #333333; font-family: 'TikTok Sans'; font-size: {self.s(60)}px; font-weight: bold; border: none; background: transparent;")

    def set_time(self, seconds):
        self.lbl_num.setText(str(seconds))

class PaymentPopup(QDialog):
    def __init__(self, parent=None, is_dark=False, scale_func=None, p_type="card"):
        super().__init__(parent)
        self.s = scale_func if scale_func else (lambda x: x) 
        ...
        self.setStyleSheet(f"QDialog {{ background-color: {bg}; border: {self.s(5)}px solid {border}; border-radius: {self.s(30)}px; }} QLabel {{ color: {fg}; font-size: {self.s(30)}pt; font-weight: 600; border: none; font-family: 'Pretendard', sans-serif; }}")
        
        self.btn_close = QPushButton("X", self)
        self.btn_close.setGeometry(self.width() - self.s(60), self.s(20), self.s(40), self.s(40))
        self.btn_close.clicked.connect(self.reject)
        self.btn_close.setStyleSheet(f"QPushButton {{ color: #999; font-family: 'Pretendard'; font-size: {self.s(30)}px; font-weight: bold; background: transparent; border: none; }} QPushButton:hover {{ color: red; }}")

        layout = QVBoxLayout(self); layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        icon_char, text_msg = "💳", "카드를 꽂아주세요"
        if p_type == "cash": icon_char, text_msg = "💵", "지폐를 투입구에\n넣어주세요"
        elif p_type == "coin": icon_char, text_msg = "🪙", "코인을 투입해주세요"
        elif p_type == "coupon": icon_char, text_msg = "🎫", "쿠폰을 확인 중입니다"
        lbl_icon = QLabel(icon_char); lbl_icon.setAlignment(Qt.AlignmentFlag.AlignCenter); lbl_icon.setStyleSheet(f"font-size: {self.s(80)}px; margin-bottom: {self.s(20)}px; color: {fg};")
        lbl_text = QLabel(text_msg); lbl_text.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(lbl_icon); layout.addWidget(lbl_text)